...
/// @endcond
{
 public:
  ...
  template<typename T> void AddOffset(voffset_t field, uoffset_t off) {
    if (!off) return;  // An offset of 0 means NULL, don't store.
    AddElement(field, ReferTo(off), static_cast<uoffset_t>(0) );
  }
